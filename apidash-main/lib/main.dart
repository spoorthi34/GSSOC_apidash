...
Future<bool> initApp(
  ...
  try {
   Logger.debug("initializeUsingPath: $initializeUsingPath");
    Logger.debug("workspaceFolderPath: ${settingsModel?.workspaceFolderPath}");
    final openBoxesStatus = await initHiveBoxes(
      initializeUsingPath,
      settingsModel?.workspaceFolderPath,
    );
    Logger.debug("openBoxesStatus: $openBoxesStatus");
    if (openBoxesStatus) {
      await autoClearHistory(settingsModel: settingsModel);
    }
    return openBoxesStatus;
  } catch (e) {
    Logger.error("initApp failed due to $e");
    return false;
  }
}
